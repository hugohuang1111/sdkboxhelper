...
func Install(staging, force bool) error {
	...
	env.AddEnvVar("SDKBOX_HOME", sdkboxHome)
	fmt.Println("sdkbox installed, please type `sdkbox -h` in terminal (maybe need reopen terminal or restart to make env var enable)")
	// switch runtime.GOOS {
	// case "windows":
